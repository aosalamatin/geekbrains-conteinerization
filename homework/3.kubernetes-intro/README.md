...
Для подключения к игре вам нужно выполнить **kubectl port-forward**, и используйте VNC клиент. 
Пароль для подключения - **idbehold**. Уточнить имя пода `kubedoom` при порт-форвардинге

```
kubectl port-forward --address 0.0.0.0 pod/kubedoom-5c955ff877-22p6r 5900:5900 -n kubedoom
```
